...
describe("Contract: Vesting", () => {
  ...
  describe("Deployment", () => {
    it("Should assign correct addresses", async () => {
      ...
      expect(await vesting.governanceRegistry()).to.equal(governanceRegistry.address);
    });
    it("Deployer, team multisig, and future investors to have role VESTING_CREATOR", async () => {
      expect(await vesting.hasRole(VESTING_CREATOR, deployer.address)).to.be
        .true;
      expect(await vesting.hasRole(VESTING_CREATOR, teamMultisig.address)).to.be
        .true;
      expect(await vesting.hasRole(VESTING_CREATOR, futureInvestors.address)).to.be
      .true;
    });
    ...
    it("Should revert if future investors address is zero", async () => {
      await expect(
        Vesting.deploy(
          teamMultisig.address,
          ethers.constants.AddressZero,
  ...
  describe("Transactions", () => {
    describe("createVestingAgreement", () => {
      ...
      beforeEach(async () => {
        ...
        await token.transfer(teamMultisig.address, totalAmount);
        await token.transfer(futureInvestors.address, totalAmount);
        await token.transfer(unauthorizedSource.address, totalAmount);
      ...
      describe("Require statements", () => {
        it("Should revert when beneficiary is zero address", async () => {
          ...
          await expect(
            vesting.createVestingAgreement(
              ethers.constants.AddressZero,
              executionTimestamp,
              VESTING_PERIOD,
              totalAmount
      ...
      describe("When called by the deployer", () => {
        beforeEach(async () => {
          ...
          await vesting.createVestingAgreement(
            beneficiary,
            vestStart,
            VESTING_PERIOD,
            totalAmount
      ...
      describe("When called by the future investors contract", () => {
        beforeEach(async () => {
          currentTimestamp = (await ethers.provider.getBlock()).timestamp;
          vestDelay = oneWeek;
          vestStart = currentTimestamp + vestDelay;

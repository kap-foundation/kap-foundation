...
contract Staking is IStaking, IVotingWeightSource, AccessControlEnumerable {
    ...
    uint256 public cumulative; // cumulative rewards per weight, multiplied by {CUMULATIVE_MULTIPLIER}
    ...
    function _sync() internal {
        if (block.timestamp > syncdTo) {
            ...
            if (syncdTo < expiration && totalWeight > 0) {
                uint256 timeElapsed = block.timestamp < expiration
                    ? block.timestamp - syncdTo
                    : expiration - syncdTo;
                cumulative +=
                    (emission.rate * timeElapsed * CUMULATIVE_MULTIPLIER) /
                    totalWeight;
            }
    ...
    function claimRewards(uint256 depositId, uint256 extension) public syncd {
        ...
        uint256 rewards = (weight * cumulativeDifference) /
    ...
    function _boost(
        ...
        require(
            MIN_LOCK <= newLock && newLock <= MAX_LOCK,
            "Staking: New lock"
        );
    ...
    modifier onlyAdmin() {
        require(
            msg.sender == governanceRegistry.governance() ||
                hasRole(TEAM_MULTISIG, msg.sender),
            "Staking: Only admin"
        );
        _;
    }
     ...
     */
    function updateEmission(uint256 rate, uint256 expiration)
        external
        onlyAdmin
        syncd
    {
